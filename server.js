...
function saveDataToFile(data) {
    Object.keys(data).forEach(parameter => {
        ...
        // Add new data entry with formatted timestamp
...
app.get('/getData/', (req, res) => {
    res.json({ arduinoData: arduinoData.map(entry => ({
...
app.get('/getData/:parameter', (req, res) => {
    ...
    if (isValidParameter(requestedParameter)) {
        const parameterData = arduinoData.map(entry => ({
            timestamp: getFormattedTimestamp(),
            value: entry[requestedParameter],
        }));
...
app.get('/getData/:parameter/:duration', (req, res) => {
    ...
    if (isValidParameter(requestedParameter)) {
        ...
        res.json({ [requestedParameter]: filteredData.map(entry => ({
            timestamp: getFormattedTimestamp(entry.timestamp),
            value: entry.value,
        })) });
    } else {
        res.status(400).json({ error: 'Invalid parameter' });
...
function isValidParameter(parameter) {
    ...
    const validParameters = ['Temperature', 'Pressure', 'Light', 'PM25', 'PM10', 'PM1', 'Humidity', 'eTVOC', 'eCO2'];
    return validParameters.some(validParam => validParam.toLowerCase() === parameter.toLowerCase());
}
